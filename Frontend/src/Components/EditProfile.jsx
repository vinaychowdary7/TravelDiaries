...
const EditProfile = () => {
    ...
    const handleSubmit = (e) =>{
        ...
        if(username.length<3){
            return toast.error("Username should be atleast 3 leatters long")
        }
        if(bio.length>bioLimit){
            return toast.error(`Bio should not be morethan ${bioLimit}`)
        }
        let loadingToast = toast.loading("Upadting...");
        e.target.setAttribute("disabled",true);

        axios.post(import.meta.env.VITE_BACKEND_SERVER_DOMAIN+"update-profile",{
            username,bio,
                    ...
                    <form ref={editProfileForm}>
                        ...
                        <div className='flex flex-col lg:flex-row items-start py-10 gap-8 lg:gap-10 '>
                            <div className='max-lg:center mb-5'>
                                ...
                                <input type='file' id='uploadImg' accept='.jpeg, .png, .jpg' hidden onChange={handleProfileImgChange} />
                                <div className="relative group inline-block">
                                    <button className='btn-light mt-5 max-lg:center lg:w-full px-10' onClick={(e) => e.preventDefault()}>Tap Here</button>
                                    <span className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-dark-grey text-white  text-xs px-3 py-1 rounded shadow-lg opacity-0 transition-opacity duration-300 group-hover:opacity-100 w-full text-center break-words">
                                        Profile image updating is currently unavailable due to image storage limitations.
                                    </span>
                                </div>
                            </div>
